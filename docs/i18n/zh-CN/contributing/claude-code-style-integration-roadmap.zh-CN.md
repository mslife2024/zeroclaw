...
- 编排集中在 `src/agent/`（`loop_.rs` 负责迭代工具循环，`query_engine.rs` 负责回合级诊断、回合后钩子，以及在 `memory.auto_save` 开启时的 **`await` 记忆合并**，`compaction_pipeline.rs` / `stop_hooks.rs` 等——前者可在裁剪后生成供动态提示尾部使用的 **memory reload** 片段）；交互与 WebSocket 路径使用 `src/agent/agent.rs` 与记忆加载、系统提示。网关将 `AppState` 中的共享 `HookRunner` 传入 `Agent::from_config_with_hooks`。
