...
- **压缩：** LLM 调用前的裁剪使用 `src/agent/compaction_pipeline.rs`（命名阶段 + `history_pruner`）；裁剪完成后可生成 **memory reload** Markdown 片段（会话记忆摘要 + 可选 AutoMemory 索引）并入动态尾部；从循环接入的上下文类重试使用同一模块的辅助函数。
...
- **钩子运行器构建：** `crate::hooks::hook_runner_from_config`（`src/hooks/mod.rs`）在 `[hooks].enabled` 时注册内置钩子；只要 `memory.auto_save` 为 true 仍注册 **`MemoryConsolidationHook`**（即使钩子总开关关闭）以保持配置中的钩子名稳定；该内置处理器为**空操作**，**合并（consolidation）在主 QueryEngine / Agent 回合路径上 `await` 完成**（`query_engine.rs`、`agent.rs`），避免重复调用 LLM。启用 `[memory.layered]` 且执行合并时，分层 SessionMemory / AutoMemory 的磁盘写入仍在 `src/memory/consolidation.rs`；`src/memory/layered_context.rs` 提供待处理回合的跨写上下文。
